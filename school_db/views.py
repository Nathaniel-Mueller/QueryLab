...
def problem_two(request):
    ...
    for i in instructors:
      print(f'Full Name: {i.first_name} {i.last_name}\nHire Date: {i.hire_date}\n')
    return complete(request)
...
def problem_three(request):
    i = Instructor.objects.get(id=2)
    courses = Course.objects.filter(instructor__id=2)
    print(f'Instructor Name: {i.first_name} {i.last_name}\nCourses:')
    for c in courses:
      print(f'    - {c.name}')
    return complete(request)
...
def problem_five(request):
    Student.objects.create(first_name = "Nathaniel", last_name = "Mueller", year = 2022, gpa = 3.2)
    s = Student.objects.get(first_name = "Nathaniel")
...
def problem_six(request):
    ...
    student_id = 16
    Student.objects.filter(id=student_id).update(gpa=4.0)
...
def bonus_problem(request):
    instructors = Instructor.objects.annotate(count=Count('course')).filter(count=1)
    for i in instructors:
      print(f'Instructor Name: {i.first_name} {i.last_name}')
    return complete(request)
